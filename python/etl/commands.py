...
class PromoteSchemasCommand(SubCommand):
    ...
    def callback(self, args):
        ...
        with etl.db.log_error():
            ...
            existing_schemas = etl.data_warehouse.filter_to_existing_schemas(selected_schemas, args.from_position)
        ...
        if len(selected_schemas) == len(existing_schemas):
            logger.info("Found %d schema(s) ready in their %s position", len(selected_schemas), args.from_position)
        else:
