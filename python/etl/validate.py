...
def validate_semantics(relations: Sequence[RelationDescription], keep_going=False) -> List[RelationDescription]:
    """
    ...
    """
    with concurrent.futures.ThreadPoolExecutor(max_workers=8, thread_name_prefix="validate-parallel") as executor:
...
def validate_transforms(dsn: dict, relations: Iterable[RelationDescription], keep_going: bool = False) -> int:
    """
...
def validate_reload(schemas: Iterable[DataWarehouseSchema], relations: Iterable[RelationDescription], keep_going: bool):
    """
    ...
    for unloaded in unloaded_relations:
        try:
            ...
            if reloaded.identifier in relations_lookup:
                ...
                logger.info("Checking for consistency between '%s' and '%s'", unloaded.identifier, relation.identifier)
                ...
                if unloaded_columns != reloaded_columns:
                    ...
                    raise TableDesignValidationError(f"unloaded relation {unloaded:x} failed to match counterpart")
...
def validate_upstream_sources(
    schemas: Iterable[DataWarehouseSchema], relations: Iterable[RelationDescription], keep_going: bool = False
) -> int:
    """
...
def validate_designs(
    ...
    while not _errors_occurred.empty():
        ...
        errors.append(f"Relation '{identifier}': {message}")
    if errors:
